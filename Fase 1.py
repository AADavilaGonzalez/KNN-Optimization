...
metric_options = ['euclidean', 'manhattan', 'chebyshev'] #euclidean = distancia directa, manhattan = distancia en bloques, chebyshev = max diferencia en cualquier dimensión

results_list = []
total_iters = len(k_values) * len(weights_options) * len(metric_options)
print(f"Total de modelos a entrenar y validar: {total_iters}")
print("Nota: Esto tomará un momento porque haremos 10 validaciones por cada modelo...")
...
for k in k_values:
    for w in weights_options:
        for m in metric_options:
            ...
            # ---  VALIDACIÓN CRUZADA ---
            # Evaluamos el modelo 10 veces con diferentes partes de los datos
            # scoring='recall' prioriza minimizar falsos negativos
            # cv=10 significa 10 particiones
            cv_scores = cross_val_score(knn, X_full_scaled, y, cv=10, scoring='recall')
